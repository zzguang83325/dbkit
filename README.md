...
DBKit 在大多数 CRUD 操作上领先 GORM，**总体性能快 16.7%**。

基于 PostgreSQL 的性能测试结果（使用独立表消除缓存效应）：
...
| **总计** | **480.8784ms** | **577.5171ms** | - | - | **DBKit 快 16.7%** |

**关键优势：**
- ✅ 并发更新快 41.8%（最大优势）
- ✅ 并发删除快 21.7%，批量插入快 12.4%
- ✅ 并发单条插入快 6.7%，并发查询快 5.1%
- ✅ 在所有 5 个测试项中全面领先
- ✅ Record 模式无反射开销，并发性能优异

### 渐进式并发压力测试

| 并发协程数 | DBKit TPS | GORM TPS | DBKit成功率 | GORM成功率 | 性能优势 |
|-----------|-----------|----------|-------------|------------|----------|
| 100 | 94471 | 52529 | 100.0% | 100.0% | **DBKit快80%** |
| 300 | 90315 | 52787 | 100.0% | 100.0% | **DBKit快71%** |
| 500 | 84770 | 38402 | 100.0% | 100.0% | **DBKit快121%** |
| 1000 | 87233 | 29644 | 100.0% | 100.0% | **DBKit快194%** |
| 5000 | 78777 | 24873 | 100.0% | 100.0% | **DBKit快217%** |

📊 **[查看完整性能测试报告](examples/benchmark/benchmark_report.md)**

**测试方法：**
- 使用独立表（`benchmark_users_dbkit` 和 `benchmark_users_gorm`）消除缓存效应
- 统一驱动：两者都使用相同的 pgx/v5 PostgreSQL 驱动
- 顺序测试架构：避免同时连接数据库造成连接数翻倍
- 相同的测试条件：数据量、批量大小、测试次数
- 测试环境：Go 1.25.5, Windows/AMD64, 16 CPU 核心, PostgreSQL 192.168.10.220:5432
- 完整测试代码见 [examples/benchmark/](examples/benchmark/)

**性能优势原因：**

- **无反射开销**: Record 模式使用 `map[string]interface{}`，避免了结构体反射的性能损耗
- **轻量级设计**: 默认关闭时间戳和乐观锁检查，减少不必要的开销
- **优化的SQL生成**: 直接生成SQL语句，减少中间层处理
- **统一驱动优势**: 使用高性能的 pgx/v5 驱动，消除了驱动差异的影响
...
| PostgreSQL | github.com/jackc/pgx/v5/stdlib   | `go get github.com/jackc/pgx/v5/stdlib`   |
...
import (
	...
	_ "github.com/zzguang83325/dbkit/drivers/mysql" // MySQL 驱动
