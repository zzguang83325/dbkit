...
type Order struct {
	dbkit.ModelCache `column:"-"`
	ID               int64      `column:"id" json:"id"`                             // 订单ID
	UserID           int64      `column:"user_id" json:"user_id"`                   // 用户ID
	OrderNo          string     `column:"order_no" json:"order_no"`                 // 订单号
	TotalAmount      float64    `column:"total_amount" json:"total_amount"`         // 订单总金额
	Status           *int64     `column:"status" json:"status"`                     // 订单状态: 0-待支付, 1-已支付, 2-已发货, 3-已完成, 4-已取消
	PaymentMethod    *string    `column:"payment_method" json:"payment_method"`     // 支付方式
	ShippingAddress  *string    `column:"shipping_address" json:"shipping_address"` // 收货地址
	...
	PaidAt           *time.Time `column:"paid_at" json:"paid_at"`                   // 支付时间
	ShippedAt        *time.Time `column:"shipped_at" json:"shipped_at"`             // 发货时间
	CompletedAt      *time.Time `column:"completed_at" json:"completed_at"`         // 完成时间
}
