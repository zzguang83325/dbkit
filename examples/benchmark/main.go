...
func writeReportFile(totalDbkit, totalGorm time.Duration, overall string) {
	...
	fmt.Fprintf(f, "DBKit 默认关闭了时间戳自动更新、乐观锁和软删除功能，以获得最佳性能。如需启用这些功能：\n\n")
	fmt.Fprintf(f, "```go\n")
	fmt.Fprintf(f, "// 启用时间戳自动更新\n")
	fmt.Fprintf(f, "dbkit.EnableTimestamps()\n\n")
	fmt.Fprintf(f, "// 启用乐观锁功能\n")
	fmt.Fprintf(f, "dbkit.EnableOptimisticLock()\n\n")
	fmt.Fprintf(f, "// 启用软删除功能\n")
	fmt.Fprintf(f, "dbkit.EnableSoftDelete()\n")
	fmt.Fprintf(f, "```\n\n")
	fmt.Fprintf(f, "启用这些功能后，相关操作会有额外的处理开销，但仍然保持良好的性能。\n\n")
